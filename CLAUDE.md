...
- **SCSS**: `@import '@/colors.scss'` for the palette. `$bronze` (#c9a45a) is the accent that replaces AntD's blue throughout. The app is dark only: there is no theme switch and no `data-theme` attribute. Each file's dark values sit in `html { ... }` blocks, overriding the lighter base values above them, and `html` carries `color-scheme: dark` for scrollbars and form controls.
